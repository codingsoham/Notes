...
fun SignUpScreen(
    ...
    LaunchedEffect(authState.value) {
        when (authState.value) {
            is AuthState.Authenticated -> onAccountCreated()
    ...
    Box(modifier= Modifier.fillMaxSize(),contentAlignment = Alignment.Center) {
        Column(
            ...
            OutlinedTextField(
                ...
                label = {
                    if (authUiState.isConfirmPasswordValid) Text(stringResource(R.string.passwords_don_t_match)) else Text(
                ...
                shape = RoundedCornerShape(50),
                singleLine = true,
                isError = authUiState.isConfirmPasswordValid,
                keyboardOptions = KeyboardOptions.Default.copy(
                    imeAction = ImeAction.Done
                ),
                keyboardActions = KeyboardActions(
                    onDone = { authViewModel.checkConfirmPassword() }
                )
            )
            Button(
                onClick = {
                    authViewModel.checkConfirmPassword()
                },
                modifier = Modifier
